...
def _write_setting(category, tag, value):
    ...
    logging.log('Escrevendo {0} - {1}: {2} to advancedsettings.xml'.format(category, tag, value), level=xbmc.LOGDEBUG)
...
class AdvancedMenu:
    ...
    def show_menu(self, url=None):
        ...
        if url_response:
            ...
            logging.log("[Advanced Settings] Sem predefinições disponíveis")
        ...
        if TEMPADVANCEDFILE:
            ...
            try:
                ...
                advanced_json = None
                logging.log("[Advanced Settings] ERRO: Formato inválido para {0}.".format(TEMPADVANCEDFILE))
            ...
            if advanced_json:
                ...
                if presets and len(presets) > 0:
                    for preset in presets:
                        ...
                        if not name:
                            logging.log('[Advanced Settings] Tag ausente \'name\'', level=xbmc.LOGDEBUG)
                            continue
                        if not preseturl:
                            logging.log('[Advanced Settings] Tag ausente \'url\'', level=xbmc.LOGDEBUG)
                            continue
    ...
    def quick_configure(self):
        directory.add_file('As alterações não serão refletidas até que o Kodi seja reiniciado.', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    def show_section(self, tags):
        ...
        if len(self.tags) == 0:
            directory.add_file('Nenhuma configuração para esta categoria existe em seu atual advancedsettings.xml arquivo.', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    def _cache(self, tag, current):
        ...
        if tag == 'buffermode':
            ...
            value = tools.get_keyboard(default='{0}'.format(current), heading='Taxa de preenchimento do cache\n(Números altos causarão uso pesado de largura de banda!)')
            
        return value
            
    def _network(self, tag, current):
        msgs = {'curlclienttimeout': 'Tempo limite em segundos para libcurl (http/ftp) connections',
                'curllowspeedtime': 'Tempo em segundos para libcurl considerar uma conexão de baixa velocidade',
                'curlretries': 'Quantidade de tentativas para certas operações libcurl com falha (e.g. timeout)',
                'httpproxyusername': 'Username para autenticação de proxy básico',
                'httpproxypassword': 'Password para autenticação de proxy básico'}
    ...
    def write_advanced(self, name, url):
        ...
        if response:
            ...
            if choice == 1:
                ...
                tools.kill_kodi(msg='[COLOR {0}]A nova predefinição advancedsettings.xml foi gravada com sucesso, mas as alterações não terão efeito até que você feche o Kodi.[/COLOR]'.format(
                                   CONFIG.COLOR2))
            else:
                logging.log("[Advanced Settings] instalar cancelado")
                logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                                   "[COLOR {0}]Escrever cancelado![/COLOR]".format(CONFIG.COLOR2))
                return
        else:
            logging.log("[Advanced Settings] URL não funciona: {0}".format(url))
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                               "[COLOR {0}]URL não funciona[/COLOR]".format(CONFIG.COLOR2))
