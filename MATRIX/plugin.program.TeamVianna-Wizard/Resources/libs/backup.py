...
def cleanup_backup():
    ...
    if len(folder) == 0:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Local de backup: vazio[/COLOR]".format(CONFIG.COLOR2))
        return
    ...
    selected = dialog.select("{0}: Selecione os itens a serem removidos do 'My_Builds' pasta.".format(CONFIG.ADDONTITLE),
                             list)

    if selected == -1:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Limpeza cancelada![/COLOR]".format(CONFIG.COLOR2))
    elif selected == 0:
        if dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Você gostaria de limpar todos os itens em seu 'My_Builds' folder?[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.MYBUILDS),
                        yeslabel="[B][COLOR springgreen]Limpar[/COLOR][/B]",
                        nolabel="[B][COLOR red]Não Cancelar[/COLOR][/B]"):
            clearedfiles, clearedfolders = tools.clean_house(CONFIG.MYBUILDS)
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Arquivos removidos: [COLOR {1}]{2}[/COLOR] / Pastas:[/COLOR] [COLOR {3}]{4}[/COLOR]".format(
        ...
        if dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Você gostaria de remover [COLOR {1}]{2}[/COLOR] da pasta 'My_Builds'?[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, list[selected]) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, path),
                        yeslabel="[B][COLOR springgreen]Limpar[/COLOR][/B]",
                        nolabel="[B][COLOR red]Não, Cancelar[/COLOR][/B]"):
            ...
            if passed:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]{1} Remover![/COLOR]".format(CONFIG.COLOR2, list[selected]))
            else:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Erro ao remover {1}![/COLOR]".format(CONFIG.COLOR2, list[selected]))
        else:
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Limpeza cancelada![/COLOR]".format(CONFIG.COLOR2))
...
class Backup:
    ...
    def backup_addon_pack(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Tem certeza de que deseja criar um Addon Pack?[/COLOR]".format(CONFIG.COLOR2),
                             nolabel="[B][COLOR red]Cancelar backup[/COLOR][/B]",
                             yeslabel="[B][COLOR springgreen]Criar Pack[/COLOR][/B]"):
            if name == "":
                name = tools.get_keyboard("", "Insira um nome para o pacote de complementos compactado")
                if not name:
                    return False
            ...
            try:
                ...
                try:
                    ...
                    if self.dialog.yesno(CONFIG.ADDONTITLE,
                                         "[COLOR {0}]Não foi possível gravar no diretório de backup atual, gostaria de alterar o local?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Mudar de diretório[/COLOR][/B]",
            ...
            selected = self.dialog.multiselect(
                "{0}: Selecione os complementos que deseja adicionar ao zip.".format(CONFIG.ADDONTITLE), addonnames)
            if selected is None:
                selected = []

            logging.log(selected)
            self.progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}][B]Criando arquivo Zip:[/B][/COLOR]'.format(CONFIG.COLOR2) + '\n' + 'Por favor, aguarde')
            if len(selected) > 0:
                added = []
            ...
            self.dialog.ok(CONFIG.ADDONTITLE,"[COLOR {0}]{1}[/COLOR] [COLOR {2}]Backup bem sucedido:[/COLOR]".format(CONFIG.COLOR1, name, CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, zipname))

    def backup_build(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Tem certeza de que deseja fazer backup da build atual?[/COLOR]".format(CONFIG.COLOR2), nolabel="[B][COLOR red]Cancelar backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup Build[/COLOR][/B]"):
            if name == "":
                name = tools.get_keyboard("", "Insira um nome para o zip de build")
                if not name:
                    return False
            ...
            if not self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Você deseja incluir sua pasta addon_data?".format(CONFIG.COLOR2) + '\n' + "Este contém [COLOR {0}]ALL[/COLOR] configurações adicionais, incluindo senhas, mas também podem conter informações importantes, como atalhos de skin. Nós recomendamos [COLOR {0}]MANUALLY[/COLOR] removendo as pastas addon_data que não são necessárias.".format(CONFIG.COLOR1, CONFIG.COLOR1) + '\n' + "[COLOR {0}]{1}[/COLOR] addon_data é ignorado[/COLOR]".format(CONFIG.COLOR1, CONFIG.ADDON_ID), yeslabel='[B][COLOR springgreen]Incluir dados[/COLOR][/B]',nolabel='[B][COLOR red]Don\'t Incluir[/COLOR][/B]'):
            ...
            try:
                ...
                try:
                    ...
                    logging.log("Incapaz de criar {0}.zip".format(name), level=xbmc.LOGERROR)
                    if self.dialog.yesno(CONFIG.ADDONTITLE,
                                         "[COLOR {0}]Não foi possível gravar no diretório de backup atual, gostaria de alterar o local?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Mudar de diretório[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                        ...
                        return
            self.progress_dialog.create(CONFIG.ADDONTITLE + "[COLOR {0}]: Criando Zip[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]Criando zip de backup".format(CONFIG.COLOR2) + '\n' + "Aguarde...[/COLOR]")
            ...
            for base, dirs, files in os.walk(CONFIG.HOME):
                ...
                for file in files:
                    try:
                        ...
                        self.progress_dialog.update(int(progress), '[COLOR {0}]Criando zip de backup: [COLOR {1}]{2}[/COLOR] / [COLOR {3}]{4}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, for_progress, CONFIG.COLOR1, N_ITEM) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file))
                        fn = os.path.join(base, file)
                        if file in CONFIG.LOGFILES:
                            logging.log("[Back Up] Tipo = build: Ignore {0} - Log File".format(file))
                            continue
                        elif os.path.join(base, file) in CONFIG.EXCLUDE_FILES:
                            logging.log("[Back Up] Tipo = build: Ignore {0} - Excluded File".format(file))
                            continue
                        elif os.path.join('addons', 'packages') in fn:
                            logging.log("[Back Up] Tipo = build: Ignore {0} - Packages Folder".format(file))
                            continue
                        elif file.startswith('._') or file.lower().startswith('.ds_store'):
                            logging.log("[Back Up] Tipo = build: Ignore {0} - OSX metadata file".format(file))
                            continue
                            ...
                            if temp in CONFIG.DB_FILES:
                                if not file == db.latest_db(temp):
                                    logging.log("[Back Up] Tipo = build: Ignore {0} - DB File".format(file))
                                    continue
                        ...
                        if skipbinary:
                            logging.log("[Back Up] Tipo = build: Ignore {0} - Binary Add-on".format(file))
                            continue
                        ...
                        try:
                            ...
                            logging.log("[Back Up] Tipo = build: Incapaz de fazer backup {0}".format(file))
                        ...
                        logging.log("[Back Up] Tipo = build: Unable to backup {0}".format(file))
            ...
            if len(binaries) > 0:
                self.dialog.ok(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Os seguintes complementos foram excluídos do backup porque são específicos da plataforma:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, ', '.join(binaries)))
        ...
        if os.path.exists(CONFIG.GUISETTINGS):
            try:
                ...
                try:
                    ...
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Não podemos gravar no diretório de backup atual, você gostaria de alterar o local?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Mudar de diretório[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                        CONFIG.open_settings()
                        return
            ...
            try:
                ...
                for fold in match:
                    fd = os.path.split(fold[:-1])[1]
                    if fd not in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
                        if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Você gostaria de adicionar a seguinte pasta de skin ao arquivo Zip de correção da GUI?[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, fd),
                                             yeslabel="[B][COLOR springgreen]Adicionar Skin[/COLOR][/B]",
                                             nolabel="[B][COLOR red]Proxima Skin[/COLOR][/B]"):
                            for base, dirs, files in os.walk(os.path.join(CONFIG.ADDON_DATA, fold)):
                                files[:] = [f for f in files if f not in CONFIG.EXCLUDE_FILES]
                                for file in files:
                                    fn = os.path.join(base, file)
                            ...
                            logging.log("[Back Up] Tipo = guifix: {0} ignored".format(fold))
            except Exception as e:
            ...
            logging.log("[Back Up] Tipo = guifix: guisettings.xml not found")
        if name == "":
    ...
    def backup_theme(self, name=""):
        if not self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Você gostaria de criar um backup de tema?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR springgreen]Continue[/COLOR][/B]",
                                 nolabel="[B][COLOR red]Não Cancelar[/COLOR][/B]"):
            logging.log_notify("Theme Backup", "Cancelled!")
            return False
        if name == "":
            themename = tools.get_keyboard("", "Por favor insira um nome para o tema zip")
            if not themename:
                return False
        else:
            themename = name
        ...
        try:
            ...
            try:
                ...
                if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Não podemos gravar no diretório de backup atual, você gostaria de alterar o local?[/COLOR]".format(CONFIG.COLOR2),
                                     yeslabel="[B][COLOR springgreen]Alterar diretório[/COLOR][/B]",
                                     nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                    CONFIG.open_settings()
                    return
                else:
        ...
        try:
            if not CONFIG.SKIN not in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
                ...
                if len(match2) > 1:
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]Você gostaria de percorrer os Arquivos de textura para?[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                         yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Proxima Textures[/COLOR][/B]"):
                        for xbt in match2:
                            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Você gostaria de adicionar o arquivo de textura [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR1, CONFIG.COLOR2, xbt.replace(skinfold, "")[1:]) + '\n' + "from [COLOR {0}]{1}[/COLOR][/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                    yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                    nolabel="[B][COLOR red]Proxima Textures[/COLOR][/B]"):
                                fn = xbt
                    ...
                    for xbt in match2:
                        if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Você gostaria de adicionar o arquivo de textura [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, xbt.replace(skinfold, "")[1:]) + '\n' + "from [COLOR {0}]{1}[/COLOR][/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                nolabel="[B][COLOR red]Proxima Textures[/COLOR][/B]"):
                            fn = xbt
                            fn2 = fn.replace(CONFIG.HOME, "")
                ...
                if os.path.exists(ad_skin):
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Você gostaria de adicionar o [COLOR {1}]settings.xml[/COLOR] in [COLOR {2}]/addon_data/[/COLOR] for?".format(
                                             CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1) + '\n'+ "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                         yeslabel="[B][COLOR springgreen]Add Settings[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Proxima Settings[/COLOR][/B]"):
                ...
                if 'script.skinshortcuts' in match:
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Você gostaria de adicionar o [COLOR {1}]settings.xml[/COLOR] for [COLOR {2}]script.skinshortcuts[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
                                         yeslabel="[B][COLOR springgreen]Add Settings[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Proxima Settings[/COLOR][/B]"):
                        for base, dirs, files in os.walk(os.path.join(CONFIG.ADDON_DATA, 'script.skinshortcuts')):
                            ...
                            for file in files:
                                ...
                                zipf.write(fn, fn[len(CONFIG.HOME):], zipfile.ZIP_DEFLATED)
            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2),
                                 "[COLOR {0}]Você gostaria de incluir um [COLOR {1}]Backgrounds[/COLOR] pasta?[/COLOR]".format(
                                     CONFIG.COLOR2, CONFIG.COLOR1),
                                 yeslabel="[B][COLOR springgreen]Sim[/COLOR][/B]",
                                 nolabel="[B][COLOR red]Não[/COLOR][/B]"):
                fn = self.dialog.browse(0, 'Selecione a localização de backgrounds', 'files', '', True, False, CONFIG.HOME,
                                        False)
                    ...
                    for base, dirs, files in os.walk(fn):
                        ...
                        for file in files:
                            try:
                                ...
                                logging.log("[Back Up] Tipo = theme: Unable to backup {0}".format(file))
                                logging.log("Backup Error: {0}".format(str(e)))
                text = db.latest_db('Textures')
                if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2),
                                     "[COLOR {0}]Você gostaria de incluir o [COLOR {1}]{2}[/COLOR]?[/COLOR]".format(
                                         CONFIG.COLOR2, CONFIG.COLOR1, text),
                                     yeslabel="[B][COLOR springgreen]Sim[/COLOR][/B]",
                                     nolabel="[B][COLOR red]Não, Continue[/COLOR][/B]"):
                    zipf.write(os.path.join(CONFIG.DATABASE, text), '/userdata/Database/{0}'.format(text),
                               zipfile.ZIP_DEFLATED)
            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]'.format(CONFIG.COLOR2),
                                 "[COLOR {0}]Você gostaria de incluir algum complemento?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR springgreen]Sim[/COLOR][/B]",
                                 nolabel="[B][COLOR red]Não, Continue[/COLOR][/B]"):
                fold = glob.glob(os.path.join(CONFIG.ADDONS, '*/'))
                addonnames = []
            ...
            zipf.close()
            logging.log("[Back Up] Tipo = theme: {0}".format(str(e)))
            self.dialog.ok(CONFIG.ADDONTITLE,
                           "[COLOR {0}]{1}[/COLOR][COLOR {2}] zip do tema falhou:[/COLOR]".format(CONFIG.COLOR1, themename, CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, str(e)))
            if not tempzipname == '':
                try:
    ...
    def backup_addon_data(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Tem certeza de que deseja fazer backup do addon_data atual?[/COLOR]".format(
                                 CONFIG.COLOR2),
                             nolabel="[B][COLOR red]Cancelar Backup[/COLOR][/B]",
                             yeslabel="[B][COLOR springgreen]Backup Addon_Data[/COLOR][/B]"):
            if name == "":
                name = tools.get_keyboard("", "Por favor, insira um nome para o addon_data zip")
                if not name:
                    return False
                name = quote_plus(name)
            ...
            try:
                ...
                try:
                    ...
                    if self.dialog.yesno(CONFIG.ADDONTITLE,
                                         "[COLOR {0}]Não foi possível gravar no diretório de backup atual, gostaria de alterar o local?[/COLOR]".format(
                                             CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Alterar diretório[/COLOR][/B]",
            ...
            self.progress_dialog.create(CONFIG.ADDONTITLE + "[COLOR {0}]: Criando Zip[/COLOR]".format(CONFIG.COLOR2), "[COLOR {0}]Criando backup zip".format(CONFIG.COLOR2) + '\n' + "Aguarde...[/COLOR]")
            ...
            for base, dirs, files in os.walk(CONFIG.ADDON_DATA):
                ...
                for file in files:
                    try:
                        for_progress += 1
                        progress = tools.percentage(for_progress, N_ITEM)
                        self.progress_dialog.update(int(progress), '[COLOR {0}]Criando backup zip: [COLOR{1}]{2}[/COLOR] / [COLOR{3}]{4}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, for_progress, CONFIG.COLOR1, N_ITEM) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file))
                        fn = os.path.join(base, file)
                        if file in CONFIG.LOGFILES:
                            logging.log("[Back Up] Tipo = addon_data: Ignore {0} - Log Files".format(file))
                            continue
                        elif os.path.join(base, file) in bad_files:
                            logging.log("[Back Up] Tipo = addon_data: Ignore {0} - Cache Files".format(file))
                            continue
                        elif os.path.join('addons', 'packages') in fn:
                            logging.log("[Back Up] Tipo = addon_data: Ignore {0} - Packages Folder".format(file))
                            continue
                        elif file.endswith('.csv'):
                            logging.log("[Back Up] Tipo = addon_data: Ignore {0} - CSV File".format(file))
                            continue
                            ...
                            if temp in CONFIG.DB_FILES:
                                if not file == db.latest_db(temp):
                                    logging.log("[Back Up] Tipo = addon_data: Ignore {0} - Database Files".format(file))
                                    continue
                        try:
                            zipf.write(fn, fn[len(CONFIG.ADDON_DATA):], zipfile.ZIP_DEFLATED)
                        except Exception as e:
                            logging.log("[Back Up] Tipo = addon_data: Unable to backup {0}".format(file))
                            logging.log("Backup Error: {0}".format(str(e)))
                    except Exception as e:
                        logging.log("[Back Up] Tipo = addon_data: Unable to backup {0}".format(file))
                        logging.log("Backup Error: {0}".format(str(e)))
