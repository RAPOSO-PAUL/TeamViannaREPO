...
if sys.version_info[0] > 2:
    ...
    def open(file, mode='r', buffering=-1, encoding='utf-8', errors=None, newline=None, closefd=True, opener=None):
        if newline is not None:
            warnings.warn('nova linha não é suportada em py2')
        if not closefd:
            warnings.warn('closefd não é suportado em py2')
        if opener is not None:
            warnings.warn('abridor não é suportado em py2')
...
def convert_special(url, over=False):
    ...
    if not over:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Converter caminhos para especial: completo![/COLOR]".format(CONFIG.COLOR2))
...
def reload_fix(default=None):
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]AVISO: Às vezes, recarregar o perfil causa o travamento do Kodi. Enquanto o Kodi estiver recarregando o perfil, não pressione nenhum botão![/COLOR]".format(CONFIG.COLOR2))
...
def ascii_check(use=None, over=False):
    ...
    if use is None:
        ...
        if over:
            ...
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Do you want to [COLOR {1}]deletar[/COLOR] todos os nomes de arquivos com caracteres especiais ou você prefere apenas [COLOR {2}]escanear e ver[/COLOR] os resultados no log?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
                                   yeslabel='[B][COLOR springgreen]Deletar[/COLOR][/B]',
                                   nolabel='[B][COLOR red]Resultados[/COLOR][/B]')
    else:
        source = use
        yes = 1
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, 'Aguarde...')
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            try:
                ...
                logging.log("[ASCII Check] Caractere ilegal encontrado no arquivo: {0}".format(file))
        ...
        if progress_dialog.iscanceled():
            progress_dialog.close()
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]ASCII Check cancelada[/COLOR]".format(CONFIG.COLOR2))
            sys.exit()
    progress_dialog.close()
    afiles.close()
    afails.close()
    ...
    if total > 0:
        ...
        if yes:
            if use:
                logging.log_notify(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]ASCII Check: {1} Remover / {2} Falhou.[/COLOR]".format(CONFIG.COLOR2, f1, f2))
            else:
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]ASCII Check: Nenhum encontrado.[/COLOR]".format(CONFIG.COLOR2))
    ...
    if _is_url(url):
        try:
            ...
            if response.status_code < 300:
                ...
                logging.log("URL requer autenticação para {0}: Status code [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return 'auth'
            else:
                logging.log("A verificação de URL falhou para {0}: Status code [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return False
        except Exception as e:
            logging.log("Erro de verificação de URL para{0}: [{1}]".format(url, e), level=xbmc.LOGDEBUG)
            return False
    else:
        logging.log("O URL não é de um esquema válido: {0}".format(url), level=xbmc.LOGDEBUG)
        return False
...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        if response.status_code == 401:
            retry = dialog.yesno(CONFIG.ADDONTITLE, 'O nome de usuário ou a senha eram inválidos. Você gostaria de tentar de novo?', yeslabel='Tentar novamente', nolabel='Cancelar')
            ...
            if retry and count < 3:
                ...
                dialog.ok(CONFIG.ADDONTITLE, 'A autenticação falhou.')
                return False
